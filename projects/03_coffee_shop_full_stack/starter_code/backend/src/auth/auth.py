...
AUTH0_DOMAIN = 'tuncerm.eu.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    ...
    if not auth:
        raise AuthError({
            "code": "no_auth_header",
            "description": "Expected Authorization header, Found none!"
        }, 401)
    ...
    if len(tokens) != 2:
        raise AuthError({
            ...
            "description": "Invalid Authorization, Expected 'Bearer Token'"
        }, 401)
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            "code": "invalid_permissions_token",
            "description": "Permissions field is required."
        }, 400)
